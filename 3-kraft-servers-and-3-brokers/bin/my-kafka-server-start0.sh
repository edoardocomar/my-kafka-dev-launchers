...
$base_dir/kafka-storage.sh format --config config/my-server0.properties --cluster-id lTGdS9wQTBiBiYFwi5jiCA --ignore-formatted

export KAFKA_DEBUG=yes
export DEBUG_SUSPEND_FLAG=n
export JAVA_DEBUG_PORT=5005

exec $base_dir/kafka-run-class.sh $EXTRA_ARGS kafka.Kafka config/my-server0.properties "$@"
