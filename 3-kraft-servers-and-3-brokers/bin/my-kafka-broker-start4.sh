...
$base_dir/kafka-storage.sh format --config config/my-broker4.properties --cluster-id lTGdS9wQTBiBiYFwi5jiCA --ignore-formatted

export KAFKA_DEBUG=no
export DEBUG_SUSPEND_FLAG=n
export JAVA_DEBUG_PORT=5405

exec $base_dir/kafka-run-class.sh $EXTRA_ARGS kafka.Kafka config/my-broker4.properties "$@"
