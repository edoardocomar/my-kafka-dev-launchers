...
mkdir -p ${LOG_DIR}
$base_dir/kafka-storage.sh format --config config/my-broker.properties --cluster-id "n86-fbTvQJGDrt18x1Re9w" --ignore-formatted

export KAFKA_DEBUG=yes
export DEBUG_SUSPEND_FLAG=n
exec $base_dir/kafka-run-class.sh $EXTRA_ARGS kafka.Kafka config/my-broker.properties "$@"
